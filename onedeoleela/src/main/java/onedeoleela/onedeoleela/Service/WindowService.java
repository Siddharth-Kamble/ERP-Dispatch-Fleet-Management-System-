...
//
//    // ══════════════════════════════════════════════════════════════════
//    //  isRowEmpty — scans ALL cells col 1..16
//    //  (Floor column removed, was col 1..17, now col 1..16)
//    //
//    //  Returns true  → every cell is null / blank / whitespace
//    //                  → row is completely empty → SKIP silently
//    //
...
//        if (row == null) return true;
//
//        // ✅ CHANGED: was col 1..17, now col 1..16 (Floor column removed)
...
//            List<String> validationErrors = new ArrayList<>();
//
//            // ✅ NEW EXCEL COLUMN LAYOUT (Floor column removed):
//            // Col 0 = SR.NO
//            // Col 1 = Window Series No.
//            // Col 2 = Flat Number       ← was Col 3 (Floor was Col 2, now removed)
//            // Col 3 = Location          ← was Col 4
//            // Col 4 = W-Code No         ← was Col 5
//            // Col 5 = Job Card No       ← was Col 6
//            // Col 6 = Series            ← was Col 7
//            // Col 7 = Description       ← was Col 8
//            // Col 8 = Width             ← was Col 9
//            // Col 9 = Height            ← was Col 10
//            // Col 10 = Track Outer      ← was Col 11
//            // Col 11 = Bottom Fix       ← was Col 12
//            // Col 12 = Glass Shutter    ← was Col 13
//            // Col 13 = Mesh Shutter     ← was Col 14
//            // Col 14 = Units            ← was Col 15
//            // Col 15 = Sqft             ← was Col 16
//            // Col 16 = Remark           ← was Col 17
//
//            for (int i = 1; i <= lastRow; i++) {
//                Row row = sheet.getRow(i);
//
...
//                }
//
//                // ✅ CHANGED: Flat Number is now Col 2 (Floor column removed)
...
//            }
//
//            System.out.println("=== PASS 1 COMPLETE — validation skipped (Flat optional) ===");
//            System.out.println("=== PASS 2 STARTING — saving rows to database ===");
//
//            // ════════════════════════════════════════════════════════════
//            //  PASS 2 — SAVE
...
//                    String flatNumber = getString(row.getCell(2));
//
//                    System.out.println("PASS2 ROW " + i + " → flat: " + flatNumber);
//
//                    Flat flat = null;
//
//                    // ✅ FIX: Create or fetch Flat directly using tower context (no floor needed)
//                    if (flatNumber != null && !flatNumber.isEmpty()) {
//                        flat = getOrCreateFlatByTower(flatNumber, tower);
//                        System.out.println("PASS2 ROW " + i + " → flat ID: "
//                                + (flat != null ? flat.getFlatId() : "null"));
//                    } else {
//                        System.out.println("PASS2 ROW " + i + " → ⚠️ Flat missing (allowed)");
//                    }
//
//                    // ✅ CHANGED: all column indices shifted left by 1 (Floor column removed)
//                    Window window = new Window();
//                    window.setWindowSeriesNumber(getString(row.getCell(1)));   // Col 1 (unchanged)
//                    // Col 2 = Flat Number → resolved above
//                    window.setLocation(getString(row.getCell(3)));              // Col 3  ← was Col 4
//                    window.setWCodeNo(getString(row.getCell(4)));               // Col 4  ← was Col 5
//                    window.setJobCardNo(getString(row.getCell(5)));             // Col 5  ← was Col 6
//                    window.setSeries(getString(row.getCell(6)));                // Col 6  ← was Col 7
//                    window.setDescription(getString(row.getCell(7)));           // Col 7  ← was Col 8
//                    window.setWidth(getDouble(row.getCell(8)));                 // Col 8  ← was Col 9
//                    window.setHeight(getDouble(row.getCell(9)));                // Col 9  ← was Col 10
//                    window.setTrackOuter(getInteger(row.getCell(10)));          // Col 10 ← was Col 11
//                    window.setBottomFix(getInteger(row.getCell(11)));           // Col 11 ← was Col 12
//                    window.setGlassShutter(getInteger(row.getCell(12)));        // Col 12 ← was Col 13
//                    window.setMeshShutter(getInteger(row.getCell(13)));         // Col 13 ← was Col 14
//                    window.setUnits(getInteger(row.getCell(14)));               // Col 14 ← was Col 15
//                    window.setSqft(getDouble(row.getCell(15)));                 // Col 15 ← was Col 16
//                    window.setRemark(getString(row.getCell(16)));               // Col 16 ← was Col 17
//
//                    // Null-safe defaults for optional Integer fields
//                    if (window.getBottomFix()    == null) window.setBottomFix(0);
//                    if (window.getGlassShutter() == null) window.setGlassShutter(0);
...
//                    if (window.getUnits()        == null) window.setUnits(0);
//
//                    // Set relations — Floor is no longer set from Excel (removed column)
//                    window.setTrip(trip);
//                    window.setFloor(null);   // ✅ Floor not used from Excel anymore
//                    window.setFlat(flat);    // ✅ Flat correctly set
//
//                    windowRepository.save(window);
//                    savedCount++;
//                    System.out.println("PASS2 ROW " + i + " → ✅ WINDOW SAVED (flat: "
//                            + (flat != null ? flat.getFlatNumber() : "none") + ")");
//
//                } catch (Exception rowEx) {
//                    System.out.println("PASS2 ROW " + i + " → ❌ FAILED: " + rowEx.getMessage());
//                    rowEx.printStackTrace();
...
//    }
//
//    // ✅ NEW HELPER: Get or Create Flat using only flat number (no floor required)
//    // Looks up flat by flat number within the tower's floors
//    private Flat getOrCreateFlatByTower(String flatNumber, Tower tower) {
//        if (flatNumber == null || flatNumber.isEmpty()) return null;
//
//        // Try to find flat in any floor under this tower
...
//                System.out.println("✅ Existing flat found: " + flatNumber
//                        + " under floor: " + floor.getFloorNumber());
//                return existing.get();
//            }
//        }
//
//        // Flat not found in any floor → create a default floor (floor 0) if needed, then create flat
//        Floor defaultFloor = towerFloors.isEmpty()
//                ? getOrCreateFloor(0, tower)
//                : towerFloors.get(0);  // attach to first available floor
//
//        System.out.println("Creating new flat: " + flatNumber
//                + " under floor: " + defaultFloor.getFloorNumber());
//
//        Flat flat = new Flat();
//        flat.setFlatNumber(flatNumber);
//        flat.setFloor(defaultFloor);
//
//        Flat saved = flatRepository.save(flat);
//
//        defaultFloor.setTotalFlats(
//                defaultFloor.getTotalFlats() == null ? 1 : defaultFloor.getTotalFlats() + 1
//        );
//        floorRepository.save(defaultFloor);
//
//        System.out.println("✅ New flat saved ID: " + saved.getFlatId()
        ...
        try {
            ...
            for (int i = 1; i <= lastRow; i++) {
                try {
                    ...
                    window.setWidth(getDouble(row.getCell(8)));                 // Col 8
                    window.setHeight(getDouble(row.getCell(9)));                // Col 9
